...
# --- NVIDIA Build: OpenAI-compatible endpoint (replaces paid OpenAI) ---
NVIDIA_BASE_URL = "https://integrate.api.nvidia.com/v1"

# --- Model IDs (served free via NVIDIA NIM) ---
GENERATION_MODEL = "meta/llama-3.3-70b-instruct"          # Q&A + legal-risk JSON
VISION_MODEL     = "meta/llama-3.2-90b-vision-instruct"   # image OCR
EMBEDDING_MODEL_API = "nvidia/llama-3.2-nv-embedqa-1b-v2" # retrieval embeddings
EMBEDDING_DIMENSION = 2048                                # nv-embedqa-1b-v2 default dim

# NOTE: the embedding dimension changed (1536 -> 2048), so we index into a NEW
# collection — an Astra collection's dimension is fixed at creation and cannot be
# altered. The old "newtest" (1536-dim) collection can be deleted from the Astra UI.
VECTOR_COLLECTION_NAME = "nv_vectors_2048"
KEYWORD_INDEX_COLLECTION_NAME = "nv_keyword_index"
PROCESSED_DOCS_KEY = "v2"  # bumped so every doc re-indexes into the new embedding space
...
def initialize_services(config: Dict[str, str]) -> Dict[str, Any]:
    ...
    # NVIDIA Build exposes an OpenAI-compatible API, so we reuse the OpenAI SDK and
    # simply repoint base_url + key. One nvapi- key powers both LLM and embeddings.
    sync_openai_client = openai.OpenAI(
        api_key=config["nvidia_api_key"], base_url=NVIDIA_BASE_URL
    )
...
async def extract_text_from_image_async(file_path: str, services: Dict[str, Any]) -> Tuple[str, int]:
    """Extracts text from an image using a NVIDIA vision-language model."""
    print("👁️ Extracting text from image using NVIDIA Vision model...")
    with open(file_path, "rb") as image_file:
        base64_image = base64.b64encode(image_file.read()).decode('utf-8')

    # NVIDIA VLMs take the image inline as an <img> data-URI embedded in the text
    # content. This works for images up to ~180KB of base64; larger images must be
    # uploaded via the NVCF assets API first (not handled here).
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "If no text is present, say nothing. "
...
def get_embeddings_sync(
    texts: List[str], client: openai.OpenAI, input_type: str = "query"
) -> List[List[float]]:
    """Synchronously generates embeddings for a list of texts.

    NVIDIA's nv-embedqa models are asymmetric: use input_type='query' for search
    queries and input_type='passage' for indexed documents. This helper defaults
    to 'query' because it is only used at search time.
    """
...
async def get_embeddings_batch_async(
    ...
    batch_size: int = 200, input_type: str = "passage"
) -> List[List[float]]:
    """Generates embeddings for a list of texts in batches asynchronously.

    Defaults to input_type='passage' since this is used to embed document chunks
    at indexing time. batch_size is conservative because the hosted NIM embedding
    endpoint caps how many inputs it accepts per request — lower it further if you
    see a 400 'too many inputs' error.
    """
    all_embeddings = []
    tasks = []
...
async def process_and_index_docx(file_path: str, document_id: str, services: Dict[str, Any]):
    ...
    if not full_text.strip():
        ...
        return
    # 🔹 Automatic language detection & translation
    translation_context = await translate_if_needed_async(full_text)
    if translation_context:
        full_text += translation_context
...
async def process_and_index_url(url: str, document_id: str, services: Dict[str, Any]):
    ...
    async with aiohttp.ClientSession() as session:
        full_text = await scrape_and_clean_url(session, url)
    if not full_text:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for filename in os.listdir(temp_dir):
            ...
            try:
                ...
                print(f"   -> ⚠️ Error processing '{filename}' inside ZIP: {e}")
...
async def get_final_contexts_for_queries(
    ...
    for i, candidate_contexts in enumerate(all_candidate_contexts):
        ...
        if len(candidate_contexts) < k_rerank:
            print(f"Skipping reranker for query '{query[:30]}...': not enough candidates ({len(candidate_contexts)}).")
...
async def get_llm_answer_async(query: str, context_chunks: List[str], services: Dict[str, Any]) -> str:
    ...
    if not context_chunks:
        print(f"No context found for query: '{query}'. Returning default message.")
        return "Could not find relevant information in the document to answer this question."

    print(f"\n======================\n LLM Context for Question: {query}")
...
# --- SECTION 9: LEGAL RISK DETECTION FOR SPONSORSHIP AGREEMENTS ---

# Short keys mapped to full risk category queries.
RISK_CATEGORY_KEYS = {
    "termination": "What are the termination and cancellation clauses, penalties, and conditions under which either party can exit?",
    "exclusivity": "What are the exclusivity restrictions, non-compete obligations, and limitations on working with competitors?",
    "liability": "What are the indemnification, liability limitation, and insurance requirements?",
    "ip": "What are the intellectual property rights, brand usage terms, and content approval requirements?",
    "payment": "What are the payment terms, fee structures, royalties, revenue sharing, and financial penalties?",
    "performance": "What are the performance obligations, deliverables, milestones, and consequences of non-performance?",
    "dispute": "What are the force majeure, dispute resolution, governing law, and arbitration clauses?",
    "confidentiality": "What are the confidentiality, non-disclosure, and data protection obligations?",
    "renewal": "What are the renewal, auto-renewal, and amendment terms and conditions?",
    "morality": "What are the morality clauses, reputation protection terms, and grounds for immediate termination?",
}

# Reverse lookup: full query string -> short key
_QUERY_TO_KEY = {query: key for key, query in RISK_CATEGORY_KEYS.items()}

# The list of queries used for retrieval (derived from the mapping)
LEGAL_RISK_QUERIES = list(RISK_CATEGORY_KEYS.values())


async def _analyze_clauses_with_llm(
    context_chunks: List[str], services: Dict[str, Any], risk_category: str
) -> str:
    """
    Sends retrieved clause contexts to the LLM for structured risk analysis.
    ...
    context = "\n\n---\n\n".join(context_chunks)
    prompt = f"""You are an expert legal risk analyst specializing in sponsorship and commercial agreements.

Your task: Analyze the following contract excerpts for the risk category "{risk_category}" and identify ALL risky, ambiguous, or potentially harmful clauses.

For EACH risky clause you find, provide a structured analysis. If no risky clauses exist for this category, return an empty JSON array.

**OUTPUT FORMAT** — respond ONLY with a JSON array, no markdown, no explanation outside the JSON:
[
  {{
    "clause_text": "The exact or near-exact text of the risky clause from the contract",
    "risk_category": "{risk_category}",
    "risk_level": "high | medium | low",
    "explanation": "Why this clause is risky, ambiguous, or potentially harmful to the sponsee/weaker party",
    "recommendation": "Specific actionable suggestion to mitigate the risk or negotiate better terms"
  }}
]

**ANALYSIS GUIDELINES:**
1. Focus on clauses that are one-sided, vague, overly broad, or that expose one party to disproportionate risk.
2. Flag missing protections that should be present (e.g., no cap on liability, no cure period before termination).
3. Identify ambiguous language that could be interpreted against the weaker party.
4. Consider standard market practice — flag deviations that disadvantage one side.
5. "high" risk = significant financial exposure or loss of key rights. "medium" = notable concern needing negotiation. "low" = minor issue worth flagging.
6. If the context contains no relevant clauses for this category, return an empty array: []
...
**JSON RESPONSE:**"""
    ...
    try:
        ...
        logtime(f"Legal risk LLM analysis for '{risk_category[:40]}'", start)
        return response.choices[0].message.content.strip()
    except Exception as e:
        print(f"Error in legal risk analysis for '{risk_category}': {e}")
        return "[]"


async def analyze_legal_risks(
    document_id: str, services: Dict[str, Any],
    semantic_weight: float, keyword_weight: float,
    k_semantic: int, k_keyword: int, k_rerank: int
) -> List[Dict[str, str]]:
    """
    Runs the full legal risk detection pipeline on an indexed document.
    Uses the existing hybrid retrieval + reranking pipeline with legal-risk-focused
    queries, then sends retrieved contexts through a specialized LLM prompt.
    Returns a list of structured risky clause objects.
    """
    print(f"⚖️ Starting legal risk analysis for document '{document_id}'...")
    t0 = time.perf_counter()

    # Step 1: Use hybrid retrieval + reranking for each risk query
    queries_with_doc = [(q, document_id) for q in LEGAL_RISK_QUERIES]
    final_contexts_per_query = await get_final_contexts_for_queries(
        queries=queries_with_doc, services=services,
        semantic_weight=semantic_weight, keyword_weight=keyword_weight,
        k_semantic=k_semantic, k_keyword=k_keyword, k_rerank=k_rerank
    )
    logtime("Legal risk retrieval complete", t0)

    # Step 2: Run LLM analysis in parallel for each risk category
    analysis_tasks = [
        _analyze_clauses_with_llm(contexts, services, query)
        for query, contexts in zip(LEGAL_RISK_QUERIES, final_contexts_per_query)
    ]
    raw_results = await asyncio.gather(*analysis_tasks)
    logtime("Legal risk LLM analysis complete", t0)

    # Step 3: Parse, merge, and deduplicate — clauses appearing in multiple
    # categories get a combined list of category keys instead of being dropped.
    clause_map = {}  # dedup_key -> clause dict (with risk_categories as list)

    for query, raw in zip(LEGAL_RISK_QUERIES, raw_results):
        category_key = _QUERY_TO_KEY.get(query, query)
        try:
            ...
            if isinstance(parsed, list):
                for clause in parsed:
                    dedup_key = clause.get("clause_text", "")[:100]
                    if not dedup_key:
                        continue
                    if dedup_key in clause_map:
                        # Clause already seen — merge this category key into it
                        existing = clause_map[dedup_key]
                        if category_key not in existing["risk_categories"]:
                            existing["risk_categories"].append(category_key)
                        # Escalate risk level if the new one is higher
                        risk_priority = {"high": 0, "medium": 1, "low": 2}
                        new_level = clause.get("risk_level", "low")
                        if risk_priority.get(new_level, 3) < risk_priority.get(existing["risk_level"], 3):
                            existing["risk_level"] = new_level
                            existing["explanation"] = clause.get("explanation", existing["explanation"])
                            existing["recommendation"] = clause.get("recommendation", existing["recommendation"])
                    else:
                        # First time seeing this clause — store with category as a list
                        clause_map[dedup_key] = {
                            "clause_text": clause.get("clause_text", ""),
                            "risk_categories": [category_key],
                            "risk_level": clause.get("risk_level", "low"),
                            "explanation": clause.get("explanation", ""),
                            "recommendation": clause.get("recommendation", ""),
                        }
        except (json.JSONDecodeError, TypeError) as e:
            print(f"Could not parse LLM risk output: {e}")
            continue

    all_risky_clauses = list(clause_map.values())

    # Step 4: Sort by risk level (high > medium > low)
    risk_order = {"high": 0, "medium": 1, "low": 2}
    all_risky_clauses.sort(key=lambda c: risk_order.get(c.get("risk_level", "low"), 3))

    logtime(f"Legal risk analysis complete — {len(all_risky_clauses)} risky clauses found", t0)
    return all_risky_clauses
